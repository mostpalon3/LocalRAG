...
- **Generate Responses:** Use an AI model (via Ollama) to provide concise answers based on the retrieved context.
...
This project uses Ollama locally for LLM and embeddings.
...
If you change the embedding model, update `OLLAMA_EMBED_DIMENSIONS` to match that model and reset/re-index the datastore.
